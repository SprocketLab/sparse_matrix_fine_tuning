...
from src.create_bert import get_glue_metrics
...
def build_model(cfg: DictConfig, num_labels: int):
    if cfg.name == 'hf_bert':
        ...
        metrics = get_glue_metrics(num_labels)
        model =  HuggingFaceModel(model, tokenizer, use_logits=True, metrics=metrics )
        return model
