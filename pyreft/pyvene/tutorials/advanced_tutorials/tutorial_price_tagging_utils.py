...
def pricing_tag_game_example_sampler(
    ...
    instruction = (
        f"Please say yes only if it costs between {lower_bound_str} and {upper_bound_str} dollars, otherwise no."
    )
...
def factual_sampler(
    ...
    for _ in range(max_n_training_examples):
        if "pricing_tag" in game:
            input_ids, output_ids = pricing_tag_game_example_sampler(tokenizer, amount, lower_bound, bound_width)
...
def lower_bound_alignment_example_sampler(tokenizer, amount=None, lower_bound=None, bound_width=None):
    (
    ...
    base_amount_sample = sample_with_region(base_region, base_lower_bound_sample, base_upper_bound_sample)
    source_amount_sample = sample_with_region(source_region, source_lower_bound_sample, source_upper_bound_sample)
...
def midpoint_alignment_sampler(
    ...
    for _ in range(max_n_training_examples):
        ...
        base_lower_bound_sample, base_upper_bound_sample, base_amount_sample = pricing_tag_game_config_sampler(
            amount, lower_bound, bound_width
        )
        source_lower_bound_sample, source_upper_bound_sample, source_amount_sample = pricing_tag_game_config_sampler(
            amount, lower_bound, bound_width
        )
        ctf_label = None
