...
class CasualModelTestCase(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.values = {"A": [False, True], "B": [False, True], "C": [False, True]}

        self.parents = {"A": [], "B": [], "C": ["A", "B"]}

        self.functions = {"A": lambda: True, "B": lambda: True, "C": lambda a, b: a and b}

        self.causal_model = CausalModel(self.variables, self.values, self.parents, self.functions)

    def test_initialization(self):
        inputs = ["A", "B"]
        outputs = ["C"]
        timesteps = {"A": 0, "B": 0, "C": 1}
        equivalence_classes = {
            "C": {
                False: [{"A": False, "B": False}, {"A": False, "B": True}, {"A": True, "B": False}],
                True: [{"A": True, "B": True}],
            }
        }
    ...
    def test_run_forward(self):
        ...
        self.assertEqual(self.causal_model.run_forward(), {"A": True, "B": True, "C": True})

        # test run forward on all possible input values
        for a in [False, True]:
            for b in [False, True]:
                input_setting = {"A": a, "B": b}
                output_setting = {"A": a, "B": b, "C": a and b}
    ...
    def test_run_interchange(self):
        ...
        self.assertEqual(self.causal_model.run_interchange(base, {"B": source})["C"], True)

        # interchange intervention on output
        base = {"A": False, "B": False}
        source = {"A": True, "B": True}
        self.assertEqual(self.causal_model.run_forward(base)["C"], False)
        self.assertEqual(self.causal_model.run_interchange(base, {"B": source})["C"], False)
        self.assertEqual(self.causal_model.run_interchange(base, {"C": source})["C"], True)
    ...
    def test_generate_factual_dataset(self):
        ...
        size = 4
        factual_dataset = self.causal_model.generate_factual_dataset(size=size, sampler=sampler, return_tensors=False)
        ...
        factual_dataset_tensors = self.causal_model.generate_factual_dataset(
            size=size, sampler=sampler, return_tensors=True
        )
        ...
        self.assertTrue(torch.equal(X[0], torch.tensor([0.0, 0.0])))
        self.assertTrue(torch.equal(y[0], torch.tensor([0.0])))
