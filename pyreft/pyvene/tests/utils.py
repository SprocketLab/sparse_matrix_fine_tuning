...
if is_package_installed(package_name):
    raise RuntimeError(f"Remove your pip installed {package_name} before running tests.")
else:
...
ONE_MLP_CLEAN_RUN = (
    lambda input_dict, mlp: mlp.mlp.h[0].act(input_dict["inputs_embeds"] @ mlp.mlp.h[0].ff1.weight.T)
    @ mlp.score.weight.T
)

ONE_MLP_FETCH_W1_OUT = lambda input_dict, mlp: input_dict["inputs_embeds"] @ mlp.mlp.h[0].ff1.weight.T

ONE_MLP_FETCH_W1_ACT = lambda input_dict, mlp: mlp.mlp.h[0].act(input_dict["inputs_embeds"] @ mlp.mlp.h[0].ff1.weight.T)

ONE_MLP_WITH_W1_OUT_RUN = lambda w1_out, mlp: mlp.mlp.h[0].act(w1_out) @ mlp.score.weight.T
...
def GPT2_SELF_ATTENTION_RUN(self_attn, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS):
    ...
    head_attention_value_output, attn_weights = self_attn._attn(head_query, head_key, head_value)
    ...
    attn_value_output = self_attn._merge_heads(head_attention_value_output, self_attn.num_heads, self_attn.head_dim)
    attn_value_output = DO_GPT2_INTERVENTION(f"{i}.attention_value_output", attn_value_output, INTERVENTION_ACTIVATIONS)
...
def GPT2_MLP_RUN(mlp, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS):
    ...
    hidden_states_act = DO_GPT2_INTERVENTION(f"{i}.mlp_activation", hidden_states_act, INTERVENTION_ACTIVATIONS)
...
def GPT2_BLOCK_RUN(block, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS):
    ...
    hidden_states = DO_GPT2_INTERVENTION(f"{i}.attention_input", hidden_states, INTERVENTION_ACTIVATIONS)
    CACHE_ACTIVATIONS[f"{i}.attention_input"] = hidden_states

    attn_outputs = GPT2_SELF_ATTENTION_RUN(block.attn, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS)

    attn_outputs = DO_GPT2_INTERVENTION(f"{i}.attention_output", attn_outputs, INTERVENTION_ACTIVATIONS)
    ...
    hidden_states = DO_GPT2_INTERVENTION(f"{i}.mlp_input", hidden_states, INTERVENTION_ACTIVATIONS)
    CACHE_ACTIVATIONS[f"{i}.mlp_input"] = hidden_states

    feed_forward_hidden_states = GPT2_MLP_RUN(block.mlp, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS)
...
def GPT2_RUN(gpt2, input_ids, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS):
    ...
    for i, block in enumerate(gpt2.transformer.h):
        hidden_states = DO_GPT2_INTERVENTION(f"{i}.block_input", hidden_states, INTERVENTION_ACTIVATIONS)
        CACHE_ACTIVATIONS[f"{i}.block_input"] = hidden_states

        hidden_states = GPT2_BLOCK_RUN(block, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS)

        hidden_states = DO_GPT2_INTERVENTION(f"{i}.block_output", hidden_states, INTERVENTION_ACTIVATIONS)
