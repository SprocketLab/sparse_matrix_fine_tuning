...
class IntervenableBasicTestCase(unittest.TestCase):
    ...
    def test_lazy_demo(self):
        ...
        pv_gpt2 = pv.IntervenableModel(
            {"layer": 0, "component": "mlp_output", "source_representation": torch.zeros(gpt2.config.n_embd)},
            model=gpt2,
        )

        intervened_outputs = pv_gpt2(
            base=tokenizer("The capital of Spain is", return_tensors="pt"), unit_locations={"base": 3}
        )
    ...
    def test_less_lazy_demo(self):
        ...
        config = pv.IntervenableConfig(
            [
                {"layer": _, "component": "mlp_output", "source_representation": torch.zeros(gpt2.config.n_embd)}
                for _ in range(4)
            ],
            mode="parallel",
        )
    ...
    def test_input_corrupt_multi_token(self):
        ...
        config = pv.IntervenableConfig({"layer": 0, "component": "mlp_input"}, pv.AdditionIntervention)
    ...
    def test_trainable_backward(self):
        ...
        config = pv.IntervenableConfig(
            {"layer": 8, "component": "block_output", "low_rank_dimension": 1}, pv.LowRankRotatedSpaceIntervention
        )
    ...
    def test_reprs_collection(self):
        ...
        config = pv.IntervenableConfig(
            {"layer": 10, "component": "block_output", "intervention_type": pv.CollectIntervention}
        )

        pv_gpt2 = pv.IntervenableModel(config, model=gpt2)

        collected_activations = pv_gpt2(
            base=tokenizer("The capital of Spain is", return_tensors="pt"), unit_locations={"sources->base": 3}
        )[0][-1]
    ...
    def test_reprs_collection_after_intervention(self):
        ...
        config = pv.IntervenableConfig(
            {"layer": 8, "component": "block_output", "intervention_type": pv.VanillaIntervention}
        )

        config.add_intervention({"layer": 10, "component": "block_output", "intervention_type": pv.CollectIntervention})
    ...
    def test_new_intervention_type(self):
        ...
        pv_gpt2 = pv.IntervenableModel({"intervention_type": MultiplierIntervention}, model=gpt2)
        intervened_outputs = pv_gpt2(
            base=tokenizer("The capital of Spain is", return_tensors="pt"), unit_locations={"base": 3}
        )
    ...
    def test_recurrent_nn(self):
        ...
        pv_gru = pv.IntervenableModel(
            {"component": "cell_output", "unit": "t", "intervention_type": pv.ZeroIntervention}, model=gru
        )

        rand_t = torch.rand(1, 10, gru.config.h_dim)

        intervened_outputs = pv_gru(base={"inputs_embeds": rand_t}, unit_locations={"base": 3})
    ...
    def test_lm_generation(self):
        ...
        pv_tinystory = pv.IntervenableModel(
            [
                {"layer": _, "component": "mlp_output", "intervention_type": pv.AdditionIntervention}
        ...
        _, intervened_story = pv_tinystory.generate(prompt, source_representations=emb_happy, max_length=32)
    ...
    def test_save_and_load(self):
        ...
        # run with new intervention type
        pv_gpt2 = pv.IntervenableModel({"intervention_type": pv.ZeroIntervention}, model=gpt2)
    ...
    def test_intervention_skipping(self):
        ...
        # skipping 1, 2 and 3
        _, pv_out1 = pv_gpt2(base, [None, None, source], {"sources->base": ([None, None, [[4]]], [None, None, [[4]]])})
        _, pv_out2 = pv_gpt2(base, [None, source, None], {"sources->base": ([None, [[4]], None], [None, [[4]], None])})
        _, pv_out3 = pv_gpt2(base, [source, None, None], {"sources->base": ([[[4]], None, None], [[[4]], None, None])})
    ...
    def test_new_model_type(self):
        ...
        t5 = T5ForConditionalGeneration.from_pretrained("google/flan-t5-small", config=config, cache_dir=self._test_dir)
        ...
        pv.type_to_module_mapping[type(t5)] = {
            "mlp_output": ("encoder.block[%s].layer[1]", pv.models.constants.CONST_OUTPUT_HOOK),
            "attention_input": ("encoder.block[%s].layer[0]", pv.models.constants.CONST_OUTPUT_HOOK),
        }
        ...
        pv_t5 = pv.IntervenableModel(
            {"layer": 0, "component": "mlp_output", "source_representation": torch.zeros(t5.config.d_model)}, model=t5
        )
    ...
    def test_path_patching(self):
        ...
        def path_patching_config(layer, last_layer, component="head_attention_value_output", unit="h.pos"):
            intervening_component = [{"layer": layer, "component": component, "unit": unit, "group_key": 0}]
            restoring_components = []
            if not component.startswith("mlp_"):
                restoring_components += [{"layer": layer, "component": "mlp_output", "group_key": 1}]
            for i in range(layer + 1, last_layer):
                restoring_components += [
                    {"layer": i, "component": "attention_output", "group_key": 1},
                    {"layer": i, "component": "mlp_output", "group_key": 1},
                ]
            intervenable_config = pv.IntervenableConfig(intervening_component + restoring_components)
            return intervenable_config

        _, tokenizer, gpt2 = pv.create_gpt2(cache_dir=self._test_dir)

        pv_gpt2 = pv.IntervenableModel(path_patching_config(4, gpt2.config.n_layer), model=gpt2)
    ...
    def test_multisource_parallel(self):
        ...
        config = pv.IntervenableConfig(
            [{"layer": 0, "component": "mlp_output"}, {"layer": 2, "component": "mlp_output"}], mode="parallel"
        )
        pv_gpt2 = pv.IntervenableModel(config, model=gpt2)
        ...
        config = pv.IntervenableConfig(
            [{"layer": 0, "component": "block_output", "subspace_partition": [[0, 128], [128, 256]]}] * 2,
            intervention_types=pv.VanillaIntervention,
            # act in parallel
            mode="parallel",
        )
    ...
    def test_multisource_serial(self):
        ...
        config = pv.IntervenableConfig(
            [{"layer": 0, "component": "mlp_output"}, {"layer": 2, "component": "mlp_output"}], mode="serial"
        )
        pv_gpt2 = pv.IntervenableModel(config, model=gpt2)
        ...
        config = pv.IntervenableConfig(
            [
                {"layer": 0, "component": "block_output", "subspace_partition": [[0, 128], [128, 256]]},
                {"layer": 2, "component": "block_output", "subspace_partition": [[0, 128], [128, 256]]},
            ],
            intervention_types=pv.VanillaIntervention,
            # act in parallel
            mode="serial",
    ...
    def test_customized_intervention_function_get(self):
        ...
        pv_gpt2 = pv.IntervenableModel(
            {"layer": 10, "component": "attention_weight", "intervention_type": pv.CollectIntervention}, model=gpt2
        )

        base = "When John and Mary went to the shops, Mary gave the bag to"
        collected_attn_w = pv_gpt2(
            base=tokenizer(base, return_tensors="pt"), unit_locations={"base": [h for h in range(12)]}
        )[0][-1][0]
        ...
        pv_gpt2 = pv.IntervenableModel(
            {"component": "h[10].attn.attn_dropout.input", "intervention": pv_patcher}, model=gpt2
        )
    ...
    def test_customized_intervention_function_zeroout(self):
        ...
        # define the component to zero-out
        pv_gpt2 = pv.IntervenableModel(
            {"component": "h[0].mlp.output", "intervention": lambda b, s: b * mask}, model=gpt2
        )
        # run the intervened forward pass
        intervened_outputs_fn = pv_gpt2(base=tokenizer("The capital of Spain is", return_tensors="pt"))
        torch.allclose(intervened_outputs[1].last_hidden_state, intervened_outputs_fn[1].last_hidden_state)
