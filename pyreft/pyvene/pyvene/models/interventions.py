...
from .layers import LowRankRotateLayer, RotateLayer
...
class BoundlessRotatedSpaceIntervention(TrainableIntervention, DistributedRepresentationIntervention):
    ...
    def __init__(self, **kwargs):
        ...
        rotate_layer = RotateLayer(self.embed_dim)
        self.rotate_layer = torch.nn.utils.parametrizations.orthogonal(rotate_layer)
        self.intervention_boundaries = torch.nn.Parameter(torch.tensor([0.5]), requires_grad=True)
        self.temperature = torch.nn.Parameter(torch.tensor(50.0))
        self.intervention_population = torch.nn.Parameter(torch.arange(0, self.embed_dim), requires_grad=False)
    ...
    def set_intervention_boundaries(self, intervention_boundaries):
        self.intervention_boundaries = torch.nn.Parameter(torch.tensor([intervention_boundaries]), requires_grad=True)
    ...
    def forward(self, base, source, subspaces=None):
        ...
        boundary_mask = torch.ones(batch_size, device=base.device).unsqueeze(dim=-1) * boundary_mask
...
class SigmoidMaskRotatedSpaceIntervention(TrainableIntervention, DistributedRepresentationIntervention):
    ...
    def __init__(self, **kwargs):
        ...
        self.masks = torch.nn.Parameter(torch.tensor([100.0] * self.embed_dim), requires_grad=True)
        self.temperature = torch.nn.Parameter(torch.tensor(50.0))
...
class PCARotatedSpaceIntervention(BasisAgnosticIntervention, DistributedRepresentationIntervention):
    ...
    def __init__(self, **kwargs):
        ...
        self.pca_mean = torch.nn.Parameter(torch.tensor(pca_mean, dtype=torch.float32), requires_grad=False)
        self.pca_std = torch.nn.Parameter(torch.tensor(pca_std, dtype=torch.float32), requires_grad=False)
        self.trainable = False
...
class NoiseIntervention(ConstantSourceIntervention, LocalistRepresentationIntervention):
    ...
    def __init__(self, **kwargs):
        ...
        noise_level = kwargs["noise_leve"] if "noise_leve" in kwargs else 0.13462981581687927
