    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, data_item in enumerate(tqdm(task_dataset)):
            ...
            # tokenize
            args = (
                (data_item[sentence1_key],)
                if sentence2_key is None
                else (data_item[sentence1_key], data_item[sentence2_key])
            )
            base_input_ids = tokenizer(
                *args, max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt"
            )["input_ids"][0]
            ...
            intervention_locations = self.get_intervention_locations(
                last_position=last_position, first_n=first_n, last_n=last_n, pad_mode="last", **kwargs
            )
        ...
        **kwargs,
        ...
        for i, data_item in enumerate(tqdm(task_dataset)):
            ...
            if task == "commonsense":
                ...
                base_input = (
                    base_prompt + data_item["answer"].replace("####", "The final answer is: ") + tokenizer.eos_token
                )
            else:
