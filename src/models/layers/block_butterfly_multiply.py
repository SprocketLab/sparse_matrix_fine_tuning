...
def block_butterfly_multiply(twiddle, input, increasing_stride=True, output_size=None):
    """
    ...
    input = (
        F.pad(input, (0, block_size * n - input_size)) if input_size < block_size * n else input[:, :, : block_size * n]
    )
    ...
    for block in range(nblocks):
        for idx in range(log_n):
            ...
            t = rearrange(
                twiddle[:, block, idx], "s (diagblk stride) i j k l -> s diagblk i j stride k l", stride=stride
            )
            output_reshape = rearrange(
                output, "b s (diagblk j stride l) -> b s diagblk j stride l", stride=stride, j=2, l=block_size
            )
            output = torch.einsum("s d i j t k l, b s d j t l -> b s d i t k", t, output_reshape)
...
def block_butterfly_factor_multiply(twiddle, input, factor_idx, increasing_stride=True, output_size=None):
    ...
    stride = 1 << log_stride
    # shape (nstacks, n // (2 * stride), 2, 2, stride, block_size, block_size)
    t = rearrange(twiddle[:, idx], "s (diagblk stride) i j k l -> s diagblk i j stride k l", stride=stride)
    output_reshape = rearrange(
        output, "b s (diagblk j stride l) -> b s diagblk j stride l", stride=stride, j=2, l=block_size
    )
    output = torch.einsum("s d i j t k l, b s d j t l -> b s d i t k", t, output_reshape)
