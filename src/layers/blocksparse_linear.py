...
class BlockSparseLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, sparsity_config, bias=True, backend="triton", weight_decay=True):
        """
        ...
        layout = self.sparsity_config.make_layout(self.out_features_extended, self.in_features_extended)
        ...
        if self.backend == "huggingface":
            self.weight = nn.Parameter(rearrange(self.weight, "nnz blksz blksz1 -> (nnz blksz1) blksz").contiguous())
        elif self.backend == "triton":
            self.weight = nn.Parameter(rearrange(self.weight, "nnz blksz blksz1 -> 1 nnz blksz blksz1"))
        if not weight_decay:
    ...
    def set_weights_from_dense_init(self, dense_init_fn_):
        ...
        with torch.no_grad():
            self.weight.copy_(
                sparsify_tensor(
                    rearrange(dense_weight, "o i -> 1 o i"), rearrange(self.layout, "o_blk i_blk -> 1 o_blk i_blk")
                )
            )
    ...
    def get_ops(self):
        if self.backend not in self.ops_cache:
            if self.backend == "triton":
                matmul_dds_op = matmul(self.layout.cpu(), self.block_size, "dds", trans_a=False, trans_b=True)
...
class FlatBlockButterflySparsityConfig:
    ...
    def make_layout(self, out_features, in_features):
        ...
        if self.shuffle:
            ...
            layout = rearrange(layout, "m (group c_per_group) -> m (c_per_group group)", group=ngroups)
        # Convert from (butterfly_size, butterfly_size) mask to (out_features, in_features) mask
        layout = repeat(
            layout,
            "b b1 -> (b f) (b1 f1)",
            f=out_features // self.butterfly_size,
            f1=in_features // self.butterfly_size,
        )
