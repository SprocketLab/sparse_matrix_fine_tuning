...
class MyAwesomeTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.model.named_parameters() if not any(nd in n for nd in no_decay) and not any(nd in n for nd in large_lr)],
            ...
            print(f"Setting lr {new_lr} and weight decay {new_decay} for {large_lr} ")
    ...
    #     """
    #     Assign larger lr to zero-init layers (scaler, etc.) to avoid gradient vanishing
    #     """
    ...
    #         }
    #     ]
    #     print(f"Setting larger lr({new_lr}) for {large_lr} ")
    #     self.optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=self.args.learning_rate, eps=self.args.adam_epsilon)
