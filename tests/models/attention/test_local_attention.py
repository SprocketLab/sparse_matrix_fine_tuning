...
class TestLocalAttention:
    ...
    def test_local_dot_product(self):
        ...
        from fast_transformers.local_product import (
            local_dot_product,
    ...
    def test_output(self, local_context, softmax_temp, device):
        ...
        local_attn = LocalAttention(local_context, softmax_temp=softmax_temp, attention_dropout=0.0).to(device)
        ...
        assert torch.all(
            torch.isclose(A_local_sum, torch.ones_like(A_local_sum))
            | torch.isclose(A_local_sum, torch.zeros_like(A_local_sum))
        )
        assert torch.all((A_local >= 1e-6).sum(dim=-1) <= local_context)
        assert torch.allclose(out_local, torch.einsum("bhts,bshd->bthd", A_local, v), rtol=1e-6, atol=1e-7)
