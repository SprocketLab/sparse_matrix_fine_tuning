...
class TestSBLocalAttention:
    ...
    def test_output(self, causal, local_context, nb_features, softmax_temp, device):
        ...
        key_padding_mask = LengthMask(
            torch.randint(low=k_seqlen // 4, high=k_seqlen, size=(batch_size,), device=device), max_len=k_seqlen
        )
        sblocal_attn = SBLocalAttention(
            local_context, embed_dim, nb_features, softmax_temp=softmax_temp, attention_dropout=0.0, causal=causal
        ).to(device)
        ...
        assert torch.all(
            torch.isclose(A_sblocal_sum, torch.ones_like(A_sblocal_sum), atol=1e-2)
            | torch.isclose(A_sblocal_sum, torch.zeros_like(A_sblocal_sum), atol=1e-2)
        )
        assert torch.allclose(out_sblocal, torch.einsum("bhts,bshd->bthd", A_sblocal, v), rtol=1e-3, atol=1e-3)
        ...
        A_full_unnormalized.masked_fill_(rearrange(~key_padding_mask.bool_matrix, "b s -> b 1 1 s"), 0.0)
        ...
        assert torch.allclose(
            A_sblocal_unnormalized.masked_select(local_mask),
        ...
        rel_error = ((A_sblocal_unnormalized - A_full_unnormalized) / A_full_unnormalized.clamp_min_(1e-6)).abs().mean()
        if device == "cuda":
        ...
        if nb_features > 10000 and local_context > 10 and (softmax_temp is None or softmax_temp < 1.0):
