...
class TestSBBlockSparseAttention:
    ...
    def test_output(self, causal, block_size, num_local_blocks, nb_features, softmax_temp, device):
        ...
        sbblocksparse_attn = SBBlockSparseAttention(
            sparsity_config,
            embed_dim,
            nb_features,
            ...
            causal=causal,
            max_seq_length=max(q_seqlen, k_seqlen),
        ).to(device)
        ...
        assert torch.allclose(out_sblocal, torch.einsum("bhts,bshd->bthd", A_sbblocksparse, v), rtol=1e-3, atol=1e-3)
        ...
        assert torch.allclose(
            A_sbblocksparse_unnormalized.masked_select(blocksparse_mask),
        ...
        rel_error = (
            ((A_sbblocksparse_unnormalized - A_full_unnormalized) / A_full_unnormalized.clamp_min_(1e-6)).abs().mean()
        )
        if device == "cuda":
