...
from src.models.attention.blocksparse_utils import (
    densify_tensor,
    mask_tensor,
    sparsify_tensor,
)
...
class TestBlockSparseAttention:
    ...
    def test_output(self, block_size, num_local_blocks, dim, softmax_temp, device):
        ...
        assert torch.all(
            torch.isclose(A_bs_sum, torch.ones_like(A_bs_sum)) | torch.isclose(A_bs_sum, torch.zeros_like(A_bs_sum))
        )
        assert torch.allclose(out_bs, torch.einsum("bhts,bshd->bthd", A_bs, v), rtol=1e-6, atol=1e-7)
