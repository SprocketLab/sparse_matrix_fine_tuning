...
class TestReformerAttention:
    ...
    def test_output(self, n_hashes, bucket_size, softmax_temp, device):
        ...
        seed_cpu_cuda(seed)
        # attn_mask = None
        attn_mask = FullMask(torch.randint(low=0, high=2, size=(seqlen, seqlen), dtype=torch.bool, device=device))
        # key_padding_mask = None
        key_padding_mask = LengthMask(
            torch.randint(low=0, high=seqlen, size=(batch_size,), device=device), max_len=seqlen
        )
        ...
        assert torch.allclose(out_reformer, torch.einsum("bhts,bshd->bthd", A_reformer, v), rtol=1e-5, atol=1e-6)
