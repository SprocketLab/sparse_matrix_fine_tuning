...
class TestSBSmyrfAttention:
    ...
    def test_output(self, causal, n_hashes, n_clusters, nb_features, softmax_temp, device):
        ...
        assert torch.all(
            torch.isclose(A_sbsmyrf_sum, torch.ones_like(A_sbsmyrf_sum), atol=1e-2)
        ...
        if nb_features < 1000:
            assert torch.allclose(out_sbsmyrf, torch.einsum("bhts,bshd->bthd", A_sbsmyrf, v), rtol=1e-3, atol=1e-3)
        ...
        rel_error = ((A_sbsmyrf_unnormalized - A_full_unnormalized) / A_full_unnormalized.clamp_min_(1e-6)).abs().mean()
        if device == "cuda":
        ...
        if nb_features < 1000:
            assert torch.allclose(
                A_sbsmyrf_unnormalized.masked_select(smyrf_mask),
