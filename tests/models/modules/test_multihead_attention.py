...
class TestMultiheadAttention:
    ...
    def test_output(self, same_qk, same_kv, dropout, bias, add_bias_kv, add_zero_attn, batch_first, device):
        seed = 2357
        embed_dim = 21
        num_heads = 3
        batch_size = 5
        ...
        seed_cpu_cuda(seed)
        attn_mask = FullMask(torch.randint(low=0, high=2, size=(q_seqlen, k_seqlen), dtype=torch.bool, device=device))
        ...
        for training in [True, False]:
            ...
            output_pt, attn_pt = mha_pt(
                q, k, v, attn_mask=~attn_mask.bool_matrix, key_padding_mask=~key_padding_mask.bool_matrix
            )
            seed_cpu_cuda(seed + 1)
            mha.train(training)
            ...
            assert output.shape == (
                (batch_size, q_seqlen, embed_dim) if batch_first else (q_seqlen, batch_size, embed_dim)
            )
            if attn is not None:
                assert attn.shape == (batch_size, num_heads, q_seqlen, k_seqlen + int(add_bias_kv) + int(add_zero_attn))
    ...
    @pytest.mark.parametrize("same_kv", [True, False])
    @pytest.mark.parametrize("same_qk", [True, False])
    def test_kdim_vdim(self, same_qk, same_kv, dropout, bias, add_bias_kv, add_zero_attn, batch_first, device):
