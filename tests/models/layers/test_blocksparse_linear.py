...
class TestBlockSparseLinear:
    ...
    def test_backends(self, in_features, out_features):
        ...
        bs_linear_hf = BlockSparseLinear(
            in_features, out_features, sparsity_config, bias=True, backend="huggingface"
        ).to("cuda")
        with torch.no_grad():
            bs_linear_hf.weight.copy_(rearrange(bs_linear_triton.weight, "1 nnz blksz blksz1 -> (nnz blksz1) blksz"))
            bs_linear_hf.bias.copy_(bs_linear_triton.bias)
        out_triton = bs_linear_triton(x)
        grad = torch.randn_like(out_triton)
        grad_x_triton, grad_weight_triton = torch.autograd.grad(out_triton, (x, bs_linear_triton.weight), grad)
        ...
        assert torch.allclose(
            rearrange(grad_weight_triton, "1 nnz blksz blksz1 -> (nnz blksz1) blksz"),
            grad_weight_hf,
            rtol=1e-4,
            atol=1e-5,
        )
    ...
    def test_output(self, in_features, out_features, block_size):
        ...
        with torch.no_grad():
            if in_features % block_size != 0 or out_features % block_size != 0:
                ...
                sparse_mask = repeat(sparse_mask, "p r -> (p blksz) (r blksz1)", blksz=block_size, blksz1=block_size)
            ...
            weight_sparse = sparsify_tensor(rearrange(weight_dense, "d2 d1 -> 1 d2 d1"), bs_linear.layout)
            layout = rearrange(bs_linear.layout, "d1 d2 -> 1 d1 d2")
            assert torch.allclose(
                densify_tensor(weight_sparse, layout)[:, :, :out_features, :in_features] * bs_fake_linear.sparse_mask,
            ...
            if bs_linear.backend == "triton":
                ...
                bs_linear.weight.copy_(rearrange(weight_sparse, "1 nnz blksz blksz1 -> (nnz blksz1) blksz"))
        ...
        grad_x_slow, grad_weight_slow = torch.autograd.grad(out_slow, (x, bs_fake_linear.weight), grad)
        ...
        if bs_linear.backend == "huggingface":
            grad_weight = rearrange(
                grad_weight, "(nnz blksz1) blksz -> 1 nnz blksz blksz1", blksz=block_size, blksz1=block_size
            )
        grad_weight_dense = densify_tensor(grad_weight, layout)
        assert torch.allclose(
            grad_weight_dense, pad_to_multiple(grad_weight_slow, multiple=block_size, dims=(0, 1)), rtol=1e-4, atol=1e-5
        )
