...
# @Wenxuan: ignore this, not monarch
def test_block_butterfly_multiply(block_size, log_n, increasing_stride, nblocks, nstacks, device):
    # set seed
    torch.random.manual_seed(0)
    n = 1 << log_n
    batch_size = 3
    twiddle = torch.randn(nstacks, nblocks, log_n, n // 2, 2, 2, block_size, block_size, device=device)
    input = torch.randn(batch_size, nstacks, block_size * n, device=device)
    increasing_stride = True
    output_size = None
    output = block_butterfly_multiply(twiddle, input, increasing_stride=increasing_stride, output_size=output_size)
...
def test_block_butterfly_factor_multiply(block_size, log_n, increasing_stride, nstacks, device):
    ...
    for idx in range(log_n):
        output_factor = block_butterfly_factor_multiply(
            twiddle[:, 0], output_factor, idx, increasing_stride=increasing_stride
        )
