...
class TestPathFinder:
    ...
    def test_dims(self, use_tar_dataset, resolution, level, sequential, to_int, val_split, test_split):
        batch_size = 57
        seed = 2357
        data_dir = Path(os.getenv("DATA_DIR", current_dir.parent.parent / "data")) / "pathfinder"
        if use_tar_dataset:
            data_dir = data_dir / f"pathfinder{resolution}.tar"
        datamodule = PathFinder(
            data_dir,
            resolution,
            level,
        ...
        assert (
            len(datamodule.dataset_train) + len(datamodule.dataset_val) + len(datamodule.dataset_test)
        ) == dataset_len
        ...
        for loader in [train_loader] + (
            ([] if val_split == 0.0 else [val_loader]) + ([] if test_split == 0.0 else [test_loader])
        ):
