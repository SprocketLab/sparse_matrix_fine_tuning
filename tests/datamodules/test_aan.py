...
class TestAAN:
    ...
    def test_dims(self, append_bos, append_eos):
        ...
        for loader in [train_loader, val_loader, test_loader]:
            ...
            if append_eos:
                assert torch.all(x1[torch.arange(batch_size), lengths1 - 1] == datamodule.vocab["<eos>"])
                assert torch.all(x2[torch.arange(batch_size), lengths2 - 1] == datamodule.vocab["<eos>"])
        ...
        if not append_bos and not append_eos:
            for loader in [train_loader, val_loader, test_loader]:
                ...
                print(
                    f"""Sequence1 length distribution: min {l1.min().item()}, max {l1.max().item()},
                        mean {l1.float().mean().item()}, stddev {l1.float().std().item()}"""
                )
                print(
                    f"""Sequence2 length distribution: min {l2.min().item()}, max {l2.max().item()},
