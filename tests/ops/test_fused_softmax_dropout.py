...
def test_softmax_dropout(seqlen, dtype):
    ...
    attention_mask_bool = repeat(torch.arange(seqlen, device=device), "s -> b s", b=batch_size) < lengths
    ...
    torch.random.manual_seed(0)
    out, dropout_mask = _fused_softmax_dropout.apply(x, dropout_prob, attention_mask, True)
