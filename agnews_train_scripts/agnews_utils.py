...
def prep_data(dataset_id, tokenizer):
    ...
    tokenize = lambda batch: tokenizer(
        batch[input_key], padding=True, truncation=True, max_length=256, return_tensors="pt"
    )
...
def run_trainer(trainer, test_dataset, precision="fp16", device="cuda"):
    ...
    with torch.autocast(device, cache_enabled=False, dtype=dtype):
        ...
        print(
            "fine-tuned roberta accuracy: ",
            round(accuracy_score(test_dataset["label"], finetuned_roberta_predictions), 3),
        )
