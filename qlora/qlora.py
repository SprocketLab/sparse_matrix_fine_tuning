...
class ModelArguments:
    ...
    trust_remote_code: Optional[bool] = field(
        default=False, metadata={"help": "Enable unpickling of arbitrary code in AutoModelForCausalLM#from_pretrained."}
    )
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    do_tune: Optional[bool] = field(default=None, metadata={"help": "Whether to do Hyperparam tuning using ASHA."})
    ...
    save_total_limit: int = field(
        default=3, metadata={"help": "How many checkpoints to save before the oldest is overwritten"}
    )
    profile: bool = field(default=False, metadata={"help": "Profile the training"})
...
def train():
    ...
    print(args)
    # TODO: explicitly call wandb init?
    os.environ["WANDB_RUN_GROUP"] = get_run_group("qlora")
    os.environ["WANDB_PROJECT"] = "llama-mmlu"
    training_args.output_dir = join(training_args.output_dir, os.environ["WANDB_RUN_GROUP"])
    print(f"Results will be saved in {training_args.output_dir}")
