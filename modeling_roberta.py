...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None, peft_config=None):
        ...
        if self.peft_config == None:
            ...
            (rank, alpha, dropout, bias) = (peft_config['lora_r'],
                                            peft_config['lora_alpha'],
                                            peft_config['lora_dropout'],
                                            peft_config['lora_bias']
                                            )
            self.query = lora.Linear(config.hidden_size, self.all_head_size, rank, alpha, dropout, bias=bias)
            self.key = nn.Linear(config.hidden_size, self.all_head_size)
            self.value = lora.Linear(config.hidden_size, self.all_head_size, rank, alpha, dropout, bias=bias)
...
class RobertaAttention(nn.Module):
    def __init__(self, config, peft_config=None, position_embedding_type=None):
        super().__init__()
        self.self = RobertaSelfAttention(config, position_embedding_type=position_embedding_type, peft_config=peft_config)
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        peft_keys = ["lora", "lora_alpha", "lora_dropout", "lora_r", "lora_bias", "monarch", "rank", "nblocks", "layers_to_replace"]
        self.peft_config = {key:config.__dict__[key] for key in peft_keys if key in config.__dict__}
